...
if(isset($_GET['delete-featured'])){
    $blog_id = (int) $_GET['bid'];
    $bFeaturedImageRemoved = $blog->delete_featured_image ($blog_id);
    if($bFeaturedImageRemoved){
        echo "<div class='alert alert-success text-center'>Your featured image has been removed successfully.</div>";
    }
            ...
            <div class="form-group blog_content_editor">
                <div class="form-row">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-12 col-md-6">
                            <?php if (is_file($current_dir . '/uploads/' . $blogdata->blog_featured_image)):?>
                            <a href="<?=admin_url('admin.php?action=edit&type=editblog&bid=' . $blog_id . '&delete-featured=1');?>"><i class="fas fa-trash-alt"></i></a>
                            <a class="fancybox" data-lightbox="featured-image" data-title="" href="<?php echo upload_url($blogdata->blog_featured_image); ?>" ><img class="img-thumbnail img-responsive m-0 mb-2" width="100" height="100" src="<?php echo upload_url($blogdata->blog_featured_image); ?>" alt="" /></a>
                            <?php endif; ?>
                            <div class="custom-control custom-checkbox" id="hide_banner">
                                <input type="checkbox" class="custom-control-input" id="hide_banner_input" name="hide_banner" value="1" <?php echo checked($blogdata->hide_banner, 1); ?>>
                                <label class="custom-control-label" for="hide_banner_input">Hide Featured Image on Blog Post</label>
                            </div>
                        </div>
                    </div>
