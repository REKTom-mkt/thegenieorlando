...
if (isset($_POST['bulk_submit'])) {
    ...
    if ($blog_bulk_action != '') {
        if ($blog_bulk_action == 'Delete') {
            ...
            if ($status) {
                echo "<div class='alert alert-success'>Blog(s) deleted.</div>";
                redirect_withdelay_script(admin_url('admin.php?action=manage&type=blog'));
            }
        }
        if ($blog_bulk_action == 'Published') {
            $ustatus = $blog->update_status($bloglist, $blog_bulk_action);
            if ($ustatus) {
                echo "<div class='alert alert-success'>Blog(s) status updated to Published.</div>";
            }
        }
        if ($blog_bulk_action == 'Draft') {
            $ustatus = $blog->update_status($bloglist, $blog_bulk_action);
            if ($ustatus) {
                echo "<div class='alert alert-success'>Blog(s) status updated to Draft.</div>";
            }
        }
    } else {
...
if (isset($_GET['bid']) && isset($_GET['action2']) && ($_GET['action2']) == 'delete') {
    ...
    if ($_SESSION['user_role'] == 'administrator') {
        if (isset($_GET['confirm']) && ($_GET['confirm'] == 1)) {
            ...
            if ($status) {
                echo "<div class='alert alert-success'>Blog post(s) deleted successfully.</div>";
                redirect_withdelay_script(admin_url('admin.php?action=manage&type=blog'));
            } else {
                echo "<div class='alert alert-danger'>Blog post(s) not deleted. Try again.</div>";
            }
        } else {
