...
function _site_url() {
    ...
    if (defined('SITEURL') && !empty(SITEURL)) {
        return SITEURL;
    }

    //Check session
    if (isset($_SESSION['blogData']['siteUrl'])) {
        return $_SESSION['blogData']['siteUrl'];
    }
    ...
    //Save to session
    $_SESSION['blogData']['siteUrl'] = $siteUrl;
...
function social_share_links($title = '', $content = '', $image = '', $url = '') {
    ...
    <div class="social_share_option">
        <!--Facebook-->
        <div class="fb-share-button" data-href="<?= $url; ?>" data-layout="button" data-size="large" style="vertical-align: top;"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($url); ?>&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore">Share</a></div>
        <!--/Facebook-->
        <!--Twitter-->
        <div id="twitter-share-button" style="display: inline-block"><a href="https://twitter.com/share?ref_src=twsrc%5Etfw" class="twitter-share-button" data-size="large" data-text="<?= $content; ?>" data-show-count="false">Tweet</a><script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script></div>
        <!--/Twitter-->
        <!--LinkedIn-->
        <div id="linkedin-share-button" style="height: 33px;display: inline-block;vertical-align: middle;"><script type="IN/Share" data-url="<?= $url; ?>"></script></div>
        <!--/LinkedIn-->
    </div>
    <?php
    return ob_get_clean();
...
{
    if(is_file(rtrim(ABSPATH, 'blog') . '404.html')){
